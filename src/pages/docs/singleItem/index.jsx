...
const SingleItem = ({sidebaropen}) => {
 ...
 useEffect(()=>{
    setState((prev)=>{
      return{
        ...prev,
        requestSchema: JSON.stringify(state.content?.requestBody?.content)?.split("/")[JSON.stringify(state.content?.requestBody?.content)?.split("/").length -1 ]?.replace(/[^a-z0-9]/gi, '')?.trim(' ')
      }
    }
...
//schemaBody
 useEffect(()=>{
   if(apiData?.components?.schemas){
     const schemaToUse = Object.entries(apiData?.components?.schemas)?.find(entry=> entry[0] === state.requestSchema)
     setState((prev)=>{
       return{
         ...prev,
           ...
           { state.paramDesc && <span>{state?.paramDesc}</span> }
                  ...
                  <div key={Math.random()}>
                    ...
                    {<span className='codeCardComment'>
                              {state.paramsList?.map?.(i=>{
                                return i?.split(":")[0] === item?.name ? ` //${i?.split(":")[1].trim("")}` : ""
                              })}
                    </span>}
                  </div>
                )
              ...
              { state?.content?.requestBody &&
               <div>
                ...
                <div className='returnSubHeader'>
                  ...
                  <circle cx="16" cy="16" r="8" stroke="white" stroke-width="1" fill="#e5989b" />
                ...
                <div className='codeCard'> 
                    <div className='codeCardKey'>
                        {"{"}
                    </div>
                    { state?.schemaBody?.[1]?.properties &&
                      Object.entries(state?.schemaBody?.[1]?.properties)?.map?.(([key, value])=>(
                        <div key={Math.random()}>
                          <span className={`codeCardKey ${state.requiredReqList.includes(key) && `required`}`}>
                              {`"${key}"`}: {""}
                          </span>
                          <span className='codeCardValue'>
                              {value?.type} 
                          ...
                          <span className='codeCardComment'>
                              {state.reqBodyList?.map?.(item=>{
                                return item?.split(":")[0] === key ? ` //${item?.split(":")[1].trim("")}` : ""
                              })}
                          </span>
                        </div>
                      ))
            ...
            {state?.returnBody}
            {
              ...
              Object.entries(state?.content?.responses)?.map(([key, value])=>{
               const refCheck =  JSON.stringify(value?.content)?.split("/")[JSON.stringify(value?.content)?.split("/").length -1 ]?.replace(/[^a-z0-9]/gi, '')?.trim(' ')
               const arrayBrackets = JSON.stringify(value?.content)?.split("/")[1]?.includes("array")
               const schemaToUse = Object.entries?.(apiData?.components?.schemas)?.find(entry=> entry[0] === refCheck)

               console.log("schhh", schemaToUse)
                return(
                  <>
                    ...
                    { schemaToUse?.[1]?.properties ?
                      Object.entries?.(schemaToUse?.[1]?.properties)?.map?.(([key, value])=>(
                        <div key={Math.random()}>
                          <span className='codeCardKey'>
                              {`"${key}"`}: {" "}
                          </span>
